...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  ...
  const body = (await request.json()) as IdPhotoRequestBody;

  if (!body.imageDataUri || !body.options) {
    return new Response('Thiếu dữ liệu tạo ID photo.', { status: 400 });
  }
  ...
  try {
    ...
    await env.DB
      ...
      .bind(userId, 'edit', result.usage.inputTokens, result.usage.outputTokens, result.usage.costUsd)
      .run();
    ...
    const message = error instanceof Error ? error.message : 'Đã xảy ra lỗi khi tạo ID photo.';
