...
export default function PortraitRestore() {
  ...
  const handleFile = (file: File) => {
    ...
    reader.onload = (event) => {
      ...
      setError(null);
      resetState();
      syncAnalysis(DEFAULT_PORTRAIT_ANALYSIS);
      setColorize(false);
  ...
  const startRestore = async () => {
    ...
    const clothingPrompt = replaceClothing
  ...
  const downloadUpscaledImage = () => {
    ...
    link.download = `QUANGTHOAI_RESTORE_${label}_${Date.now()}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
        ...
        <AnimatePresence mode="wait">
          {!originalImage ? (
            <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="flex h-full items-center justify-center p-4 pt-20 md:pt-24">
              <div className="group relative mx-4 flex aspect-video w-full max-w-xl cursor-pointer flex-col items-center justify-center gap-6 rounded-3xl border-2 border-dashed border-white/10 bg-white/[0.02] transition-all duration-500 hover:border-blue-500/40 hover:bg-blue-500/[0.02]" onClick={() => document.getElementById('portrait-file-input')?.click()}>
                <div className="flex h-20 w-20 items-center justify-center rounded-full bg-white/5 transition-all duration-500 group-hover:scale-110 group-hover:bg-blue-500/10"><Upload className="h-8 w-8 text-white/30 transition-colors group-hover:text-blue-400" /></div>
                <div className="px-4 text-center"><p className="text-lg font-medium text-white/80">Kéo thả ảnh vào đây</p><p className="mt-1 text-sm text-white/30">JPG, PNG, WEBP (Tối đa 10MB)</p></div>
                ...
                <aside className="w-full shrink-0 lg:w-[390px]">
                  <div className="rounded-3xl border border-white/10 bg-white/[0.03] backdrop-blur-xl lg:h-full lg:max-h-[calc(100vh-8rem)] lg:overflow-y-auto">
                    <div className="sticky top-0 z-10 border-b border-white/10 bg-[#050505]/90 px-4 py-4 backdrop-blur-xl">
                      ...
                      <p className="mt-1 text-xs text-white/40">Chỉ dùng Gemini từ Frontend, không còn backend Upscale.</p>
                    </div>

                    <div className="space-y-3 p-4">
                      {renderStepSection('analysis', '1', 'Xác nhận thông tin ảnh', 'Thiết lập metadata thủ công để prompt bám sát ảnh gốc.', analysis && (
                        <div className="space-y-5">
                          <div>
                            <p className="mb-2 text-[10px] uppercase tracking-wider text-white/30">Loại chân dung</p>
                            <div className="grid grid-cols-3 gap-2">
                              {[{ value: 'portrait_single', label: 'Người lớn' }, { value: 'child_portrait', label: 'Trẻ em' }, { value: 'elderly_portrait', label: 'Người lớn tuổi' }].map((item) => (
                                <button key={item.value} onClick={() => updateAnalysis({ photo_type: item.value })} className={`rounded-xl border px-3 py-2 text-xs font-bold transition-all ${analysis.photo_type === item.value ? 'border-blue-500/40 bg-blue-500/10 text-blue-300' : 'border-white/10 bg-white/[0.03] text-white/60 hover:bg-white/[0.06]'}`}>{item.label}</button>
                              ))}
                            </div>
                          </div>
                          <div className="grid grid-cols-2 gap-3">
                            ...
                            <label className="text-xs text-white/60">Kích thước gương mặt
                              <select value={analysis.face_sizes} onChange={(e) => updateAnalysis({ face_sizes: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white focus:outline-none">
                            ...
                            <label className="text-xs text-white/60">Mức hư hại
                              <select value={analysis.damage_severity} onChange={(e) => updateAnalysis({ damage_severity: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white focus:outline-none">
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {DAMAGE_TYPE_OPTIONS.map((type) => {
                                ...
                                return <button key={type} onClick={() => toggleDamageType(type)} className={`rounded-lg border px-2.5 py-1.5 text-[11px] font-medium transition-all ${active ? 'border-blue-500/40 bg-blue-500/10 text-blue-300' : 'border-white/10 bg-white/[0.03] text-white/60 hover:bg-white/[0.06]'}`}>{DAMAGE_TYPE_LABELS[type]}</button>;
                              })}
                            </div>
                          </div>
                      ...
                      {renderStepSection('model', '2', 'Chọn Model', 'Flash cho tốc độ, Pro cho ảnh hư hại nặng hơn.', (
                      ...
                      {renderStepSection('options', '3', 'Tùy chọn nâng cao', 'Bật lên màu, thay trang phục và bắt đầu phục hồi.', (
                        <div className="space-y-4">
                          ...
                          <button onClick={() => setReplaceClothing(!replaceClothing)} className={`w-full rounded-2xl border p-4 transition-all ${replaceClothing ? 'border-blue-500/30 bg-blue-500/10' : 'border-white/10 bg-white/[0.03] hover:bg-white/[0.06]'}`}><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3 text-left"><Shirt className={`h-5 w-5 ${replaceClothing ? 'text-blue-400' : 'text-white/30'}`} /><div><p className="text-sm font-bold">Thay trang phục</p><p className="text-[10px] text-white/40">Chỉ áp dụng cho ảnh chân dung</p></div></div><div className={`relative h-5 w-10 rounded-full ${replaceClothing ? 'bg-blue-500' : 'bg-white/10'}`}><motion.div className="absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-white" animate={{ x: replaceClothing ? 20 : 0 }} /></div></div></button>

                          <AnimatePresence>
                            {replaceClothing && (
                              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden"><div className="space-y-2 rounded-2xl border border-blue-500/15 bg-blue-500/[0.04] p-3">{[...Object.keys(CLOTHING_PRESETS), 'Tùy chỉnh'].map((opt) => (<button key={opt} onClick={() => setClothingOption(opt)} className={`w-full rounded-xl border px-3 py-2 text-left text-xs font-medium transition-all ${clothingOption === opt ? 'border-blue-500/40 bg-blue-500/10 text-blue-300' : 'border-white/10 bg-white/[0.03] text-white/55 hover:bg-white/[0.06]'}`}>{opt}</button>))}{clothingOption === 'Tùy chỉnh' && <input type="text" value={clothingText} onChange={(e) => setClothingText(e.target.value)} placeholder="Ví dụ: Áo vest đen, Áo dài xanh..." className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-xs text-white placeholder:text-white/20 focus:outline-none" />}</div></motion.div>
                            )}
                          </AnimatePresence>
                        ...
                        <div className="mt-3 space-y-3">
                          <div className="rounded-2xl border border-white/10 bg-black/20 p-3 text-xs text-white/60">
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-2"><CheckCircle2 className={`h-4 w-4 ${restoredImage ? 'text-blue-300' : 'text-white/25'}`} /><span>{restoredImage ? 'Đã có ảnh Gemini để tải xuống' : 'Chưa có ảnh Gemini'}</span></div>
