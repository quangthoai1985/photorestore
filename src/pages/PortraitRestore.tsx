...
const DEFAULT_PORTRAIT_ANALYSIS: AnalysisResult = {
  ...DEFAULT_ANALYSIS,
...
export default function PortraitRestore() {
  ...
  const handleFile = (file: File) => {
    ...
    reader.onload = (event) => {
      ...
      setError(null);
      resetState();
      syncPortraitAnalysis(DEFAULT_PORTRAIT_ANALYSIS);
      setColorize(false);
  ...
  const renderStepSection = (
    ...
    return (
      <div className={`rounded-2xl border transition-all ${isOpen ? 'border-blue-500/40 bg-blue-500/[0.06]' : 'border-white/10 bg-white/[0.03]'}`}>
        <button
          type="button"
          disabled={isLocked}
        ...
        >
          <div className={`mt-0.5 flex h-8 w-8 items-center justify-center rounded-xl border text-xs font-black ${isCompleted ? 'border-emerald-500/30 bg-emerald-500/15 text-emerald-300' : isOpen ? 'border-blue-500/30 bg-blue-500/15 text-blue-300' : 'border-white/10 bg-white/[0.03] text-white/55'}`}>
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between gap-3">
              ...
              <ChevronRight className={`h-4 w-4 shrink-0 transition-transform ${isOpen ? 'rotate-90 text-blue-300' : 'text-white/25'}`} />
            </div>
          </div>
        </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] uppercase tracking-widest text-white/30">Phục hồi Ảnh Chân Dung</p>
            </div>
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          {!originalImage ? (
            <motion.div
              ...
              className="flex h-full items-center justify-center p-4 pt-20 md:pt-24"
            >
              <div
                className="group relative mx-4 flex aspect-video w-full max-w-xl cursor-pointer flex-col items-center justify-center gap-6 rounded-3xl border-2 border-dashed border-white/10 bg-white/[0.02] transition-all duration-500 hover:border-blue-500/40 hover:bg-blue-500/[0.02]"
                onClick={() => document.getElementById('portrait-file-input')?.click()}
              >
                <div className="flex h-20 w-20 items-center justify-center rounded-full bg-white/5 transition-all duration-500 group-hover:scale-110 group-hover:bg-blue-500/10">
                  <Upload className="h-8 w-8 text-white/30 transition-colors group-hover:text-blue-400" />
                </div>
                <div className="px-4 text-center">
                  <p className="text-lg font-medium text-white/80">Kéo thả ảnh vào đây</p>
                  <p className="mt-1 text-sm text-white/30">JPG, PNG, WEBP (Tối đa 10MB)</p>
                </div>
                <label className="cursor-pointer rounded-full bg-white px-8 py-3 text-sm font-semibold text-black shadow-lg transition-colors hover:bg-blue-50" onClick={(e) => e.stopPropagation()}>
                  Chọn từ máy tính
            ...
            <motion.div
              ...
              className="h-full w-full p-4 pb-24 pt-20 md:p-8 md:pb-10 md:pt-24"
            >
                ...
                <div className="relative min-h-[320px] flex-1 overflow-hidden rounded-2xl border border-white/10 bg-black/40 lg:min-h-0">
                  {restoredImage ? (
                    ...
                    <div className="relative flex h-full w-full items-center justify-center">
                      <img src={originalImage} alt="Original Preview" className="max-h-full max-w-full object-contain" referrerPolicy="no-referrer" />
                      ...
                      {isProcessing && (
                        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-6 bg-black/60 backdrop-blur-sm">
                          <div className="relative h-20 w-20">
                            ...
                            <motion.div className="absolute inset-0 rounded-full border-4 border-t-blue-500" animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }} />
                            <div className="absolute inset-0 flex items-center justify-center">
                              <Sparkles className="h-8 w-8 text-blue-400" />
                            </div>
                          </div>
                          <div className="space-y-3 px-8 text-center">
                            <p className="text-lg font-bold tracking-wide text-white">{status.step}</p>
                            <div className="h-1.5 w-64 overflow-hidden rounded-full bg-white/10">
                              <motion.div className="h-full bg-gradient-to-r from-blue-500 to-purple-500" initial={{ width: 0 }} animate={{ width: `${status.progress}%` }} />
                            </div>
                          </div>
                        </div>
                      )}
                ...
                <aside className="w-full shrink-0 lg:w-[390px]">
                  <div className="h-full rounded-3xl border border-white/10 bg-white/[0.03] backdrop-blur-xl lg:max-h-[calc(100vh-8rem)] lg:overflow-y-auto">
                    <div className="sticky top-0 z-10 border-b border-white/10 bg-[#050505]/90 px-4 py-4 backdrop-blur-xl">
                      ...
                      <p className="mt-1 text-xs text-white/40">Sidebar step-down: hoàn tất bước trước để mở bước tiếp theo.</p>
                    </div>
                    ...
                    <div className="space-y-3 p-4">
                      {renderStepSection(
                        ...
                        'Thiết lập metadata thủ công để prompt bám sát ảnh gốc.',
                        analysis && (
                          <div className="space-y-5">
                            <div>
                              <p className="mb-2 text-[10px] uppercase tracking-wider text-white/30">Loại chân dung</p>
                              <div className="grid grid-cols-3 gap-2">
                                {[
                                  { value: 'portrait_single', label: 'Người lớn' },
                                  { value: 'child_portrait', label: 'Trẻ em' },
                                  { value: 'elderly_portrait', label: 'Người lớn tuổi' },
                                ].map((item) => (
                                  <button key={item.value} onClick={() => updateManualAnalysis({ photo_type: item.value })} className={`rounded-xl border px-3 py-2 text-xs font-bold transition-all ${analysis.photo_type === item.value ? 'border-blue-500/40 bg-blue-500/10 text-blue-300' : 'border-white/10 bg-white/[0.03] text-white/60 hover:bg-white/[0.06]'}`}>
                                    {item.label}
                                  </button>
                                ))}
                              </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              ...
                              <label className="text-xs text-white/60">Kích thước gương mặt
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {DAMAGE_TYPE_OPTIONS.map((type) => {
                                  ...
                                  return (
                                    <button key={type} onClick={() => toggleDamageType(type)} className={`rounded-lg border px-2.5 py-1.5 text-[11px] font-medium transition-all ${active ? 'border-blue-500/40 bg-blue-500/10 text-blue-300' : 'border-white/10 bg-white/[0.03] text-white/60 hover:bg-white/[0.06]'}`}>
                            ...
                            <label className="block text-xs text-white/60">Ghi chú đặc biệt
                              <textarea value={analysis.special_challenges ?? ''} onChange={(e) => updateManualAnalysis({ special_challenges: e.target.value.length > 0 ? e.target.value : null })} rows={3} placeholder="Ví dụ: áo bị mất texture mạnh, hậu cảnh là bàn ghế gỗ cần giữ chi tiết..." className="mt-2 w-full resize-none rounded-xl border border-white/10 bg-white/5 px-3 py-3 text-sm text-white placeholder:text-white/20 focus:outline-none" />
                            </label>

                            <button onClick={() => setPopupStep('model')} className="w-full rounded-2xl bg-gradient-to-r from-blue-600 to-purple-600 py-3 text-sm font-bold text-white transition-all hover:from-blue-500 hover:to-purple-500">Tiếp tục sang bước 2</button>
                          </div>
                        ),
                      )}
                      ...
                      {renderStepSection(
                        ...
                        '2',
                        'Chọn Model',
                        'Flash cho tốc độ, Pro cho ảnh hư hại nặng hơn.',
                        <div className="space-y-3">
                          <button onClick={() => setSelectedModel('gemini-3.1-flash-image-preview')} className={`w-full rounded-2xl border p-4 text-left transition-all ${selectedModel === 'gemini-3.1-flash-image-preview' ? 'border-blue-500/40 bg-blue-500/10 ring-1 ring-blue-500/20' : 'border-white/10 bg-white/[0.03] hover:bg-white/[0.06]'}`}>
                            <div className="flex items-start gap-4">
                              <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${selectedModel === 'gemini-3.1-flash-image-preview' ? 'bg-blue-500/20' : 'bg-white/5'}`}><Zap className={`h-5 w-5 ${selectedModel === 'gemini-3.1-flash-image-preview' ? 'text-blue-400' : 'text-white/30'}`} /></div>
                              <div><p className="text-sm font-bold">Gemini 3.1 Flash Image</p><p className="mt-1 text-xs text-white/40">Nhanh, hợp ảnh nhẹ đến trung bình.</p></div>
                            </div>
                          </button>
                          <button onClick={() => setSelectedModel('gemini-3-pro-image-preview')} className={`w-full rounded-2xl border p-4 text-left transition-all ${selectedModel === 'gemini-3-pro-image-preview' ? 'border-purple-500/40 bg-purple-500/10 ring-1 ring-purple-500/20' : 'border-white/10 bg-white/[0.03] hover:bg-white/[0.06]'}`}>
                            <div className="flex items-start gap-4">
                              <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${selectedModel === 'gemini-3-pro-image-preview' ? 'bg-purple-500/20' : 'bg-white/5'}`}><ShieldCheck className={`h-5 w-5 ${selectedModel === 'gemini-3-pro-image-preview' ? 'text-purple-400' : 'text-white/30'}`} /></div>
                              <div><p className="text-sm font-bold">Gemini 3 Pro Image</p><p className="mt-1 text-xs text-white/40">Chất lượng cao nhất cho ảnh khó.</p></div>
                            </div>
                          </button>
                          <button onClick={() => setPopupStep('resolution')} className="w-full rounded-2xl bg-gradient-to-r from-blue-600 to-purple-600 py-3 text-sm font-bold text-white transition-all hover:from-blue-500 hover:to-purple-500">Tiếp tục sang bước 3</button>
                        </div>,
                        activeStepIndex < 1,
                      )}
                      ...
                      {renderStepSection(
                        ...
                        '3',
                        'Chất lượng đầu ra',
                        'Chọn độ phân giải phù hợp chất lượng và tốc độ.',
                        <div className="space-y-4">
                          <div className="grid grid-cols-3 gap-3">
                            {[{ id: '1K' as ResolutionType, label: '1K', desc: '1024px' }, { id: '2K' as ResolutionType, label: '2K', desc: '2048px' }, { id: '4K' as ResolutionType, label: '4K', desc: '4096px' }].map((res) => (
                              <button key={res.id} onClick={() => setSelectedResolution(res.id)} className={`rounded-2xl border p-4 text-center transition-all ${selectedResolution === res.id ? 'border-blue-500/40 bg-blue-500/10 ring-1 ring-blue-500/20' : 'border-white/10 bg-white/[0.03] hover:bg-white/[0.06]'}`}>
                                <span className={`block text-2xl font-black ${selectedResolution === res.id ? 'text-blue-400' : 'text-white/60'}`}>{res.label}</span>
                                <span className="mt-1 block text-[10px] text-white/30">{res.desc}</span>
                              </button>
                            ))}
                          </div>
                          <button onClick={() => setPopupStep('options')} className="w-full rounded-2xl bg-gradient-to-r from-blue-600 to-purple-600 py-3 text-sm font-bold text-white transition-all hover:from-blue-500 hover:to-purple-500">Tiếp tục sang bước 4</button>
                        </div>,
                        activeStepIndex < 2,
                      )}
                      ...
                      {!restoredImage && renderStepSection(
                        ...
                        '4',
                        'Tùy chọn nâng cao',
                        'Bật lên màu, thay trang phục và khởi chạy phục hồi.',
                        <div className="space-y-4">
                          <button onClick={() => setColorize(!colorize)} className={`w-full rounded-2xl border p-4 transition-all ${colorize ? 'border-blue-500/30 bg-blue-500/10' : 'border-white/10 bg-white/[0.03] hover:bg-white/[0.06]'}`}>
                            <div className="flex items-center justify-between gap-3">
                              <div className="flex items-center gap-3 text-left"><Palette className={`h-5 w-5 ${colorize ? 'text-blue-400' : 'text-white/30'}`} /><div><p className="text-sm font-bold">Lên màu AI</p><p className="text-[10px] text-white/40">Tô màu tự nhiên cho ảnh đen trắng/sepia</p></div></div>
                              <div className={`relative h-5 w-10 rounded-full ${colorize ? 'bg-blue-500' : 'bg-white/10'}`}><motion.div className="absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-white" animate={{ x: colorize ? 20 : 0 }} /></div>
                            </div>
                          </button>

                          <button onClick={() => setReplaceClothing(!replaceClothing)} className={`w-full rounded-2xl border p-4 transition-all ${replaceClothing ? 'border-blue-500/30 bg-blue-500/10' : 'border-white/10 bg-white/[0.03] hover:bg-white/[0.06]'}`}>
                            <div className="flex items-center justify-between gap-3">
                              <div className="flex items-center gap-3 text-left"><Shirt className={`h-5 w-5 ${replaceClothing ? 'text-blue-400' : 'text-white/30'}`} /><div><p className="text-sm font-bold">Thay trang phục</p><p className="text-[10px] text-white/40">Chỉ áp dụng cho ảnh chân dung</p></div></div>
                              <div className={`relative h-5 w-10 rounded-full ${replaceClothing ? 'bg-blue-500' : 'bg-white/10'}`}><motion.div className="absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-white" animate={{ x: replaceClothing ? 20 : 0 }} /></div>
                            </div>
                          </button>
                          ...
                          <AnimatePresence>
                            {replaceClothing && (
                              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                                <div className="space-y-2 rounded-2xl border border-blue-500/15 bg-blue-500/[0.04] p-3">
                                  {[...Object.keys(CLOTHING_PRESETS), 'Tùy chỉnh'].map((opt) => (
                                    <button key={opt} onClick={() => setClothingOption(opt)} className={`w-full rounded-xl border px-3 py-2 text-left text-xs font-medium transition-all ${clothingOption === opt ? 'border-blue-500/40 bg-blue-500/10 text-blue-300' : 'border-white/10 bg-white/[0.03] text-white/55 hover:bg-white/[0.06]'}`}>{opt}</button>
                                  ))}
                                  {clothingOption === 'Tùy chỉnh' && <input type="text" value={clothingText} onChange={(e) => setClothingText(e.target.value)} placeholder="Ví dụ: Áo vest đen, Áo dài xanh..." className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-xs text-white placeholder:text-white/20 focus:outline-none" />}
                                </div>
                              </motion.div>
                            )}
                          </AnimatePresence>

                          <button onClick={startRestore} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-purple-600 py-3.5 text-sm font-bold text-white transition-all hover:from-blue-500 hover:to-purple-500">
                      ...
                      {renderStepSection(
                        ...
                        '5',
                        'Upscale',
                        restoredImage ? 'Nâng độ phân giải sau khi Gemini phục hồi xong.' : 'Bước này sẽ mở khi Gemini trả ảnh kết quả.',
                        restoredImage ? <div className="space-y-5">
                          <div>
                            <div className="mb-2 flex items-center justify-between"><p className="text-sm font-semibold">Scale factor</p><span className="text-xs text-blue-300">{upscaleScaleFactor}x</span></div>
                            <input type="range" min={2} max={4} step={1} value={upscaleScaleFactor} onChange={(e) => setUpscaleScaleFactor(Number(e.target.value))} className="w-full accent-blue-500" />
                            <p className="mt-1 text-[10px] text-white/35">2x nhanh hơn, 4x chi tiết cao hơn nhưng lâu hơn</p>
                          </div>
                          <div>
                            <div className="mb-2 flex items-center justify-between"><p className="text-sm font-semibold">Creativity</p><span className="text-xs text-purple-300">{upscaleCreativity.toFixed(2)}</span></div>
                            <input type="range" min={0} max={1} step={0.05} value={upscaleCreativity} onChange={(e) => setUpscaleCreativity(Number(e.target.value))} className="w-full accent-purple-500" />
                            <p className="mt-1 text-[10px] text-white/35">Giữ thấp để tự nhiên hơn, tăng khi muốn bù chi tiết mạnh.</p>
                          </div>
                          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                            <div className="mb-2 flex items-center justify-between"><p className="text-xs text-white/70">Trạng thái upscale</p><p className="text-xs font-semibold text-blue-300">{Math.round(upscaleProgress)}%</p></div>
                            <div className="h-1.5 w-full overflow-hidden rounded-full bg-white/10"><motion.div className="h-full bg-gradient-to-r from-blue-500 to-purple-500" initial={{ width: 0 }} animate={{ width: `${Math.max(0, Math.min(100, upscaleProgress))}%` }} /></div>
                          ...
                          <button onClick={startUpscale} disabled={isUpscaling} className="w-full rounded-2xl bg-gradient-to-r from-blue-600 to-purple-600 py-3 text-sm font-bold text-white transition-all hover:from-blue-500 hover:to-purple-500 disabled:opacity-60">{isUpscaling ? 'Đang upscale...' : 'Upscale với Crystal'}</button>
                        </div> : (
                      ...
                      <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 lg:sticky lg:bottom-4">
                        ...
                        <div className="mt-3 space-y-3">
                          <div className="rounded-2xl border border-white/10 bg-black/20 p-3 text-xs text-white/60">
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-2">
                                <CheckCircle2 className={`h-4 w-4 ${restoredImage ? 'text-blue-300' : 'text-white/25'}`} />
                                <span>{restoredImage ? 'Đã có ảnh Gemini để tải xuống' : 'Chưa có ảnh Gemini'}</span>
                              </div>
                              <span className={`rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${restoredImage ? 'border-blue-500/30 bg-blue-500/10 text-blue-300' : 'border-white/10 bg-white/[0.03] text-white/35'}`}>Gemini {restoredImage ? 'Ready' : 'Pending'}</span>
                            </div>
                            <div className="mt-2 flex items-center justify-between gap-2">
                              <div className="flex items-center gap-2">
                                <Sparkles className={`h-4 w-4 ${upscaledImage ? 'text-emerald-300' : 'text-white/25'}`} />
                                <span>{upscaledImage ? 'Đã có ảnh Upscale để tải xuống' : 'Chưa có ảnh Upscale'}</span>
                              </div>
                              <span className={`rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${upscaledImage ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300' : 'border-white/10 bg-white/[0.03] text-white/35'}`}>Upscale {upscaledImage ? 'Ready' : 'Pending'}</span>
                            </div>
                          </div>
                          ...
                          {restoredImage && (
                            <>
                              <button onClick={downloadGeminiImage} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-white px-4 py-3 text-sm font-bold text-black shadow-lg transition-all hover:bg-blue-50">
              ...
              <div className="mt-4 flex items-center gap-4">
                {geminiRestoredImage && upscaledImage && (
                  <div className="flex items-center gap-2">
                    <button onClick={() => setCompareMode('original-vs-current')} className={`rounded-full border px-3 py-1.5 text-xs font-bold transition-all ${compareMode === 'original-vs-current' ? 'border-blue-500/40 bg-blue-500/15 text-blue-300' : 'border-white/10 bg-white/[0.03] text-white/55 hover:bg-white/[0.06]'}`}>Gốc / Hiện tại</button>
                    <button onClick={() => setCompareMode('gemini-vs-upscaled')} className={`rounded-full border px-3 py-1.5 text-xs font-bold transition-all ${compareMode === 'gemini-vs-upscaled' ? 'border-purple-500/40 bg-purple-500/15 text-purple-300' : 'border-white/10 bg-white/[0.03] text-white/55 hover:bg-white/[0.06]'}`}>Gemini / Upscale</button>
                  </div>
                )}
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 20 }}
            className="fixed bottom-6 left-1/2 z-50 flex max-w-md -translate-x-1/2 items-start gap-3 rounded-2xl border border-red-500/20 bg-red-500/10 p-4 backdrop-blur-xl"
          >
